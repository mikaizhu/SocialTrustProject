...
if __name__ == "__main__":
    ...
    solution1_tf = tf_vectorizer.transform(solution1_word2str) # 174 * 7287
    ...
    solution1_doc_topic_dist = lda.transform(solution1_tf) # 174 * 42
    ...
    if 0:
    	...
    	solution2_n_clusters = max_index+2 # 加2是因为从2开始计算的聚类个数, 而max数据是从0开始计算
    ...
    # 聚类完成后，使用类中心点的向量，来表示归类为这一点的数据，即只要这个数据是归为这一类，那么就用类中心点来替换这个数据
    ...
    s2_b = [item.iloc[:base] for item in s2_a]#个数为3*solution2_n_clusters
    ...
    for classLabel in range(solution3_n_clusters):
    	...
    	for userID,record in solution3_sorted_df[solution3_sorted_df.classLabel==classLabel].iterrows():
    		...
    		if int(record.talkative) and len(temp_s)<base:#若是话痨,且stubb未满, 即每个类中只挑一个stubb
