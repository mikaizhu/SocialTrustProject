...
%%%%% Let lambda models the frequency of words
 
total_cnt_words = sum(C(3:end,:,:),3)*ones(K,1);
lambda = total_cnt_words / max(total_cnt_words);
    ...
    cvx_begin
        ...
        for nn = 1 : N
            ls_term(nn) >= square_pos(norm(P_cvx([1:2],:)*X(:,:,nn) - C([1:2],:,nn),'fro'))...
                + square_pos(norm(diag(lambda)*P_cvx(3:L,:)*X(:,:,nn) - C(3:L,:,nn),'fro'));
        end
...
%             P_cvx*ones(3,1) >= 0.01;
    ...
    % solve for lambda
%     cvx_begin
%         variable lambda_cvx(L-2)
%         variable ls_term(N)
%         for nn = 1 : N
%             ls_term(nn) >= square_pos(norm(diag(lambda_cvx)*P_cvx(3:L,:)*X(:,:,nn) - C(3:L,:,nn),'fro'));
%         end
%         minimize( sum(ls_term) )
%         subject to
%         lambda_cvx>=0;
    ...
    for nn = 1 : N
        cvx_begin
            ...
            minimize( square_pos(norm(P_cvx([1:2],:)*X_cvx_nn - C([1:2],:,nn),'fro'))...
                + square_pos(norm(diag(lambda)*P_cvx(3:L,:)*X_cvx_nn - C(3:L,:,nn),'fro')) )
            for kk = 1 : K
                if C(1,kk,nn) == 1
